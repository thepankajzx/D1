...
function InsightGuideModal({ onClose }) {
  ...
  const [activePill, setActivePill] = useState('POWER DUO');
  ...
  const archetypes = [
    {
      ...
      color: 'sky',
      badgeClass: 'bg-sky-100 text-sky-800 border-sky-300 dark:bg-sky-950/80 dark:text-sky-300 dark:border-sky-800',
      tag: 'Reinforcement',
      desc: isHinglish
        ...
        : 'When two habits mutually reinforce each other. Synergy % measures how often completing Habit A directly coincides with completing Habit B.'
    },
    {
      id: 'FOCUS DRAIN',
      label: 'FOCUS DRAIN',
    ...
    },
    {
      ...
      color: 'violet',
      badgeClass: 'bg-violet-100 text-violet-800 border-violet-300 dark:bg-violet-950/80 dark:text-violet-300 dark:border-violet-800',
      tag: 'Resilience',
      desc: isHinglish
  ...
  return (
    <div className="fixed inset-0 z-[130] flex items-center justify-center p-3 sm:p-4 bg-slate-950/75 backdrop-blur-sm animate-in fade-in duration-150">
      ...
      <div className="relative w-full max-w-lg max-h-[85vh] bg-white dark:bg-[#131722] rounded-3xl p-4 sm:p-6 shadow-2xl border border-slate-200 dark:border-slate-800 z-10 flex flex-col overflow-hidden animate-in zoom-in-95 duration-150 space-y-3">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-[10px] sm:text-[11px] text-slate-400 font-medium">
                {isHinglish ? 'Behavioral archetypes aur pill guide' : 'Interactive behavioral archetypes & pill guide'}
              </p>
            </div>
          </div>
          <button 
        ...
        {/* Interactive Archetype Toggle Pills Bar */}
        <div className="flex items-center gap-1.5 overflow-x-auto pb-1 shrink-0 select-none">
          {archetypes.map((arch) => {
            const isSelected = activePill === arch.id;
            return (
              <button
                key={arch.id}
                type="button"
                onClick={() => {
                  ...
                  setActivePill(arch.id);
                }}
                className={'px-2.5 py-1 rounded-xl text-[10px] font-black uppercase tracking-wider transition-all whitespace-nowrap cursor-pointer border ' + (isSelected ? 'bg-slate-900 text-white dark:bg-white dark:text-slate-900 border-slate-900 dark:border-white shadow-xs' : arch.badgeClass + ' hover:opacity-80')}
              >
                {arch.label}
              </button>
            );
        ...
        {/* Selected Archetype Detailed Card + All Accordion List */}
        <div className="space-y-2 overflow-y-auto pr-0.5 text-xs text-slate-700 dark:text-slate-300 leading-relaxed custom-scrollbar">
          {archetypes.map((arch) => {
            const isExpanded = activePill === arch.id;
            return (
              <div 
                key={arch.id}
                className={'rounded-2xl border transition-all overflow-hidden ' + (isExpanded ? 'bg-slate-50/90 dark:bg-[#181d2c] border-purple-300 dark:border-purple-800 ring-2 ring-purple-500/10 shadow-xs' : 'bg-white dark:bg-[#151926] border-slate-200/80 dark:border-slate-800/80 hover:border-slate-300')}
              >
                <div 
                  onClick={() => {
                    if (navigator.vibrate) navigator.vibrate(15);
                    setActivePill(prev => prev === arch.id ? null : arch.id);
                  }}
                  className="p-3 flex items-center justify-between gap-2 cursor-pointer select-none"
                >
                  <div className="flex items-center gap-2 flex-wrap min-w-0">
                    <span className={'px-2 py-0.5 rounded-md text-[9.5px] font-extrabold uppercase tracking-wider border ' + arch.badgeClass}>
                      {arch.label}
                    </span>
                  ...
                  <div className={'text-slate-400 transition-transform duration-200 shrink-0 ' + (isExpanded ? 'rotate-180 text-purple-600 dark:text-purple-400' : '')}>
                    <CaretDown size={14} weight="bold" />
                  </div>
                </div>

                {isExpanded && (
                  <div className="px-3 pb-3 pt-0 text-[11.5px] text-slate-600 dark:text-slate-300 border-t border-slate-100 dark:border-slate-800/80 mt-1 pt-2 animate-in fade-in duration-150 space-y-1.5">
                    <p>{arch.desc}</p>
                    <div className="flex items-center gap-1.5 text-[10px] font-bold text-slate-400 pt-0.5">
                      <span>Archetype Focus:</span>
                      <span className="text-purple-600 dark:text-purple-400 font-extrabold">{arch.tag}</span>
                    </div>
                  </div>
                )}
              </div>
        ...
        <div className="pt-2 border-t border-slate-100 dark:border-slate-800 shrink-0">
          <button 
            type="button" 
            onClick={onClose} 
            className="w-full py-2.5 bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 text-white rounded-2xl text-xs font-black shadow-md transition-all cursor-pointer active:scale-98"
          >

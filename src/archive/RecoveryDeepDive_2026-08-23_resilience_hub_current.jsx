...
export default function RecoveryDeepDive() {
  ...
  return (
    <div className="max-w-[1100px] mx-auto w-full space-y-4 md:space-y-6 px-1 sm:px-3 md:px-0 pb-16">
      ...
      <section className="bg-gradient-to-b from-[#140b05] via-[#0d0703] to-[#0a0502] text-white rounded-3xl p-5 sm:p-6 md:p-7 border border-orange-900/40 shadow-xl relative overflow-hidden flex flex-col gap-4 w-full">
        ...
        <div className="flex items-center justify-between gap-3 w-full relative z-10">
          <div className="flex items-center gap-2.5">
            <div className="w-8 h-8 rounded-xl bg-[#281508] border border-orange-500/30 text-orange-500 flex items-center justify-center flex-shrink-0">
          ...
          <button 
            ...
            className="w-8 h-8 sm:w-9 sm:h-9 rounded-full bg-orange-950/60 hover:bg-orange-900/80 text-orange-400 flex items-center justify-center border border-orange-500/30 cursor-pointer flex-shrink-0 transition-all active:scale-95"
        ...
        <div className="w-full bg-[#170c06]/80 hover:bg-[#1f1008] rounded-2xl border border-orange-900/40 p-4 sm:p-5 flex items-center justify-between gap-4 relative z-10 shadow-sm">
          <div className="flex items-center gap-3 sm:gap-4 min-w-0">
            <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-2xl bg-[#281508] border border-orange-500/30 text-orange-500 flex items-center justify-center flex-shrink-0">
      ...
      {/* ── 4. HOW RESILIENCE WORKS MODAL ── */}
      {showInfo && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-black/60 backdrop-blur-xs">
          <div className="fixed inset-0" onClick={handleCloseInfo} />
          <div className="relative w-full max-w-[440px] max-h-[90vh] bg-white rounded-3xl p-4 sm:p-5 shadow-2xl border border-slate-200 z-10 flex flex-col overflow-hidden">
            <div className="flex items-center justify-between pb-2.5 mb-2 border-b border-slate-100 shrink-0">
              <div className="flex items-center gap-2.5">
                <div className="w-8 h-8 rounded-xl bg-orange-50 text-orange-600 flex items-center justify-center shrink-0"><ShieldCheck size={18} weight="fill" /></div>
                <div><h4 className="font-extrabold text-sm text-slate-900 leading-tight">How Resilience Works</h4><span className="text-[10px] text-slate-400 font-medium">Quick Recovery Guide</span></div>
              </div>
              <button type="button" onClick={handleCloseInfo} className="w-8 h-8 rounded-full hover:bg-slate-100 text-slate-400 hover:text-slate-700 flex items-center justify-center cursor-pointer shrink-0"><X size={16} weight="bold" /></button>
            </div>
            <div className="space-y-2 overflow-y-auto pr-0.5">
              {[
                {n:1,t:'7-Day Window',b:'After you miss a day, a 7-day recovery window starts. We track how quickly you bounce back.'},
                {n:2,t:'Bounce-Back',b:'A bounce-back happens when you complete your habit on at least 5 of the next 7 days (70%+).'},
                {n:3,t:'Recovery Streak',b:'Consecutive misses you have successfully bounced back from. A failed recovery resets the streak.'},
                {n:4,t:'Resilience Score',b:'Your average recovery rate across all misses. Shows a dash if you have never missed.'}
              ].map(({n,t,b}) => (
                <div key={n} className="flex items-start gap-2 bg-slate-50/90 p-2.5 rounded-xl border border-slate-100">
                  <span className="w-4 h-4 rounded-full bg-orange-100 text-orange-700 font-bold text-[9px] flex items-center justify-center shrink-0 mt-0.5">{n}</span>
                  <div><strong className="text-slate-900 block font-bold text-xs leading-tight mb-0.5">{t}</strong><p className="text-slate-600 leading-snug text-xs">{b}</p></div>
                </div>
              ))}
            </div>
            <div className="mt-2.5 pt-2.5 border-t border-slate-100 shrink-0">
              <button type="button" onClick={handleCloseInfo} className="w-full py-2 bg-slate-900 hover:bg-slate-800 text-white rounded-xl text-xs font-bold cursor-pointer">Got it</button>
            </div>
          </div>
        </div>
      )}

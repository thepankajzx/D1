...
export default function RecoveryDeepDive() {
  ...
  return (
    <div className="max-w-[1100px] mx-auto w-full space-y-4 md:space-y-6 px-1 sm:px-3 md:px-0 pb-16">
      ...
      {/* ── 3. ORANGE & BLACK GRADIENT WRAPPER (RESILIENCE SCORE + TOP-RIGHT ATTRACTIVE (i) BUTTON) ── */}
      <section className="bg-gradient-to-b from-[#140b05] via-[#0d0703] to-[#0a0502] text-white rounded-3xl p-5 sm:p-6 md:p-7 border border-orange-900/40 shadow-xl relative overflow-hidden flex flex-col gap-4 w-full">
        ...
        {/* Top Row: Shield Icon + Resilience Title + Top-Right Glowing (i) Button */}
        <div className="flex items-center justify-between gap-3 w-full relative z-10">
          ...
          {/* Attractive (i) Button with subtle glow & badge */}
          <button 
            type="button" 
            onClick={() => {
            ...
            className="group px-3 py-1.5 rounded-2xl bg-gradient-to-r from-orange-500/20 via-amber-500/15 to-orange-500/10 hover:from-orange-500/30 hover:to-amber-500/20 border border-orange-500/40 hover:border-orange-400 text-orange-400 hover:text-white flex items-center gap-1.5 cursor-pointer shadow-sm hover:shadow-orange-500/20 transition-all duration-200 active:scale-95 flex-shrink-0"
            title="How Resilience Works"
          >
            <div className="w-4 h-4 rounded-full bg-orange-500/30 text-orange-300 group-hover:bg-orange-500 group-hover:text-white flex items-center justify-center transition-colors">
              <Info size={12} weight="bold" />
            </div>
            <span className="text-[11px] font-black tracking-wide uppercase">
              {isHinglish ? 'गाइड' : 'Guide'}
            </span>
          </button>
        </div>
      ...
      {/* ── 4. HOW RESILIENCE WORKS ATTRACTIVE GUIDE MODAL ── */}
      {showInfo && (
        <div className="fixed inset-0 z-[120] flex items-center justify-center p-3 sm:p-4 bg-slate-950/75 backdrop-blur-sm animate-in fade-in duration-150">
          ...
          <div className="relative w-full max-w-[480px] max-h-[90vh] bg-white dark:bg-[#131722] rounded-3xl p-5 sm:p-6 shadow-2xl border border-orange-200/80 dark:border-orange-900/50 z-10 flex flex-col overflow-hidden animate-in zoom-in-95 duration-150 space-y-4">
            
            {/* Modal Header */}
            <div className="flex items-center justify-between pb-3 border-b border-slate-100 dark:border-slate-800 shrink-0">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-2xl bg-gradient-to-br from-orange-500 to-amber-600 text-white flex items-center justify-center shadow-md shrink-0">
                  <ShieldCheck size={22} weight="fill" />
                </div>
                <div>
                  <h4 className="font-black text-base text-slate-900 dark:text-white leading-tight">
                    {isHinglish ? 'रेज़िलिएंस गाइड और बाउंस-बैक' : 'Resilience & Bounce-Back Guide'}
                  </h4>
                  <span className="text-[11px] text-slate-400 font-medium">
                    {isHinglish ? 'आदत छूटने के बाद वापसी का विज्ञान' : 'The science of habit recovery & bounce-back velocity'}
                  </span>
                </div>
              </div>
              <button 
                ...
                className="w-8 h-8 rounded-full bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 text-slate-400 hover:text-slate-700 dark:hover:text-white flex items-center justify-center cursor-pointer transition-all shrink-0"
              >
            ...
            <div className="space-y-2.5 overflow-y-auto pr-1">
              {[
                {
                  n: '1',
                  t: isHinglish ? '7-दिन की रिकवरी विंडो' : '7-Day Recovery Window',
                  b: isHinglish 
                    ? 'जब भी कोई आदत किसी दिन मिस होती है, उसी दिन से अगले 7 दिनों की एक ट्रैकिंग विंडो शुरू हो जाती है।'
                    : 'Whenever you miss a habit day, an automatic 7-day evaluation window begins to measure your comeback speed.',
                  badge: 'Step 1 • Window',
                  bg: 'bg-indigo-50/70 dark:bg-indigo-950/30 border-indigo-200/70 dark:border-indigo-900/50',
                  tcolor: 'text-indigo-700 dark:text-indigo-300',
                  icon: <CalendarCheck size={18} weight="fill" className="text-indigo-600 dark:text-indigo-400" />
                },
                {
                  n: '2',
                  t: isHinglish ? '70%+ बाउंस-बैक थ्रेशोल्ड' : '70%+ Bounce-Back Threshold',
                  b: isHinglish 
                    ? 'अगले 7 दिनों में अगर आप कम से कम 5 दिन (70%+) यह आदत पूरी करते हैं, तो इसे सफल बाउंस-बैक माना जाता है।'
                    : 'Achieving ≥70% completion (at least 5 out of 7 days) successfully converts a miss into a confirmed recovery.',
                  badge: 'Step 2 • Threshold',
                  bg: 'bg-emerald-50/70 dark:bg-emerald-950/30 border-emerald-200/70 dark:border-emerald-900/50',
                  tcolor: 'text-emerald-700 dark:text-emerald-300',
                  icon: <CheckCircle size={18} weight="fill" className="text-emerald-600 dark:text-emerald-400" />
                },
                {
                  n: '3',
                  t: isHinglish ? 'लगातार रिकवरी स्ट्रीक' : 'Consecutive Recovery Streak',
                  b: isHinglish 
                    ? 'आप बिना किसी लगातार विफलता के कितनी बार लगातार बाउंस-बैक कर पाए हैं। यह आपकी अनुशासन शक्ति को मापता है।'
                    : 'Counts how many consecutive missed days you successfully recovered from without letting a slip turn into a relapse.',
                  badge: 'Step 3 • Streak',
                  bg: 'bg-orange-50/70 dark:bg-orange-950/30 border-orange-200/70 dark:border-orange-900/50',
                  tcolor: 'text-orange-700 dark:text-orange-300',
                  icon: <Flame size={18} weight="fill" className="text-orange-600 dark:text-orange-400" />
                },
                {
                  n: '4',
                  t: isHinglish ? 'ओवरऑल रेज़िलिएंस स्कोर' : 'Overall Resilience Score %',
                  b: isHinglish 
                    ? 'आपके सभी मिस इवेंट्स में सफल रिकवरी का औसत प्रतिशत। यदि आपने कभी मिस नहीं किया, तो यह परफेक्ट रहता है।'
                    : 'Your lifetime recovery percentage across all historical slips. Reflects true mental grit and habit adaptability.',
                  badge: 'Step 4 • Score',
                  bg: 'bg-purple-50/70 dark:bg-purple-950/30 border-purple-200/70 dark:border-purple-900/50',
                  tcolor: 'text-purple-700 dark:text-purple-300',
                  icon: <TrendUp size={18} weight="fill" className="text-purple-600 dark:text-purple-400" />
                }
              ].map((card) => (
                <div key={card.n} className={'p-3 rounded-2xl border transition-all ' + card.bg}>
                  <div className="flex items-center justify-between mb-1.5">
                    <div className="flex items-center gap-2">
                      <div className="shrink-0">{card.icon}</div>
                      <strong className={'font-black text-xs ' + card.tcolor}>{card.t}</strong>
                    </div>
                    <span className="text-[9px] font-extrabold uppercase px-2 py-0.5 rounded-md bg-white/80 dark:bg-slate-900/80 border border-slate-200/50 dark:border-slate-800 text-slate-500">
                      {card.badge}
                    </span>
                  </div>
                  <p className="text-slate-600 dark:text-slate-300 text-[11px] leading-relaxed pl-6">
                    {card.b}
                  </p>
                </div>
              ))}
            ...
            <div className="pt-2 border-t border-slate-100 dark:border-slate-800 shrink-0">
              <button 
                type="button" 
                onClick={handleCloseInfo} 
                className="w-full py-2.5 bg-gradient-to-r from-orange-600 to-amber-600 hover:from-orange-500 hover:to-amber-500 text-white rounded-2xl text-xs font-black shadow-md hover:shadow-orange-500/25 transition-all cursor-pointer active:scale-98"
              >
                {isHinglish ? 'समझ गया (Got it)' : 'Understood & Continue'}
              </button>
            </div>

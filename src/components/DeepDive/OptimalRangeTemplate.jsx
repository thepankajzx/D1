...
import { getOptimalRangeAnalytics } from '../../lib/optimalRangeAnalytics';
...
export default function OptimalRangeTemplate({ habit, allSummaries, habits, dateRange }) {
  const data = useMemo(() => {
    return getOptimalRangeAnalytics(habit, allSummaries, dateRange);
  ...
  const { summary, target0, target100, targetMax100, targetMax0, unit, formatVal, fmtVal: rawFmtVal, bestDay, improvement, weekdayPattern, maxWeekdayAvg, dailyDetails, personalBests, trend, insights, weakestDay } = data;

  const targetValue = data.targetValue || (target100 ? (targetMax100 && targetMax100 !== target100 ? `${target100} - ${targetMax100}` : target100) : (habit.userTarget100 ? `${habit.userTarget100} - ${habit.targetMax || habit.userTarget100}` : 'Not Set'));
  ...
  return (
    <div className="flex flex-col gap-3 md:gap-5 w-full pb-10">
        ...
        <div className="p-3.5 md:p-5 flex items-start gap-4 w-full">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-[11px] md:text-xs text-slate-500 dark:text-slate-400 font-medium leading-snug">
              Target: <span className="text-slate-700 dark:text-slate-200 font-bold">{target100 || habit.userTarget100 || habit.target100 || 7} - {targetMax100 || habit.userTargetMax || habit.targetMax || 9}{unit ? ` ${unit}` : (habit.unit || habit.defaultUnit ? ` ${habit.unit || habit.defaultUnit}` : '')}</span>
              <span className="hidden md:inline"> &nbsp;·&nbsp; </span>
              <br className="md:hidden" />
              Scoring: <span className="text-slate-700 dark:text-slate-200">Optimal Range</span>
            </p>
          </div>
        </div>
        ...
        {showScoringInfo && (
          <div className="px-3.5 md:px-5 pb-3.5 md:pb-5">
            <div className="bg-indigo-50/50 dark:bg-indigo-950/30 rounded-xl border border-indigo-100/50 dark:border-indigo-900/40 p-3 md:p-4 text-[11px] md:text-[12px] text-slate-600 dark:text-slate-300 leading-relaxed">
              ...
              Hitting exactly <strong className="text-emerald-600">{targetValue > 0 ? fmtVal(targetValue) : 'Target'} = 100 points</strong>. 
              Moving away from the target in <strong className="text-rose-500">either direction</strong> decreases your score until it hits 0.
            </div>
          </div>
        )}
      ...
      {/* ── 3. SCORE CHART + DONUT ──────────────────────────────────────── */}
      <div id="deep-dive-charts-section-optimal" className="w-full flex flex-col gap-3">
        {/* Header */}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6 w-full">
        ...
        <div className="bg-white dark:bg-[#151a26] p-4 md:p-6 rounded-xl md:rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col w-full">
          <h3 className="text-sm font-semibold text-slate-900 mb-1">Value Pattern</h3>
          <p className="text-[10px] md:text-[11px] text-slate-400 mb-4">Average daily value by day of week</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6 w-full">
        ...
        <div className="bg-white dark:bg-[#151a26] p-4 md:p-6 rounded-xl md:rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col">
          ...
          <div className="flex flex-col gap-4 flex-1 pt-1">
            {[
              { label: 'Highest Value', value: personalBests.bestValue !== null ? fmtVal(personalBests.bestValue) + (personalBests.bestDate ? ' · ' + formatDate(personalBests.bestDate) : '') : 'N/A' },
      ...
      <div className="p-3 md:p-4 bg-indigo-50 dark:bg-indigo-950/30 rounded-xl text-center text-[11px] md:text-[12px] font-bold text-indigo-700 dark:text-indigo-300 border border-indigo-100 dark:border-indigo-900/40">
        With Optimal Range, progress means moving up. Reaching your target earns 100 points — never more than 100.
      </div>

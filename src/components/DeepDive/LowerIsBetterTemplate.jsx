...
import { getLowerIsBetterAnalytics } from '../../lib/lowerIsBetterAnalytics';
...
export default function LowerIsBetterTemplate({ habit, allSummaries, habits, dateRange }) {
  const data = useMemo(() => {
    return getLowerIsBetterAnalytics(habit, allSummaries, dateRange);
  ...
  return (
    <div className="flex flex-col gap-3 md:gap-5 w-full pb-10">
        ...
        <div className="p-3.5 md:p-5 flex items-start gap-4 w-full">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-[11px] md:text-xs text-slate-500 dark:text-slate-400 font-medium leading-snug">
              ...
              Scoring: <span className="text-slate-700 dark:text-slate-200">Lower Is Better</span>
            </p>
          </div>
        </div>
        ...
        {showScoringInfo && (
          <div className="px-3.5 md:px-5 pb-3.5 md:pb-5">
            <div className="bg-indigo-50/50 dark:bg-indigo-950/30 rounded-xl border border-indigo-100/50 dark:border-indigo-900/40 p-3 md:p-4 text-[11px] md:text-[12px] text-slate-600 dark:text-slate-300 leading-relaxed">
              ...
              Staying <strong className="text-emerald-600">at or below {targetValue > 0 ? (unit ? `${targetValue} ${unit}` : fmtVal(targetValue)) : 'your target'} = 100 pts</strong>.
              Going higher than target decreases your score. Above the max limit, score drops to 0.
              <span className="ml-2 font-semibold text-indigo-600">Move DOWN ↓ → Score goes UP</span>
            </div>
          </div>
        )}
      ...
      {/* ── 3. SCORE CHART + DONUT ──────────────────────────────────────── */}
      <div id="deep-dive-charts-section-lower" className="w-full flex flex-col gap-3">
        {/* Header */}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6 w-full">
        ...
        <div className="bg-white dark:bg-[#151a26] p-4 md:p-6 rounded-xl md:rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col">
          ...
          <div className="flex flex-col gap-4 flex-1 pt-1">
            {[
              { label: 'Lowest Usage', value: personalBests.bestValue !== null ? fmtVal(personalBests.bestValue) + (personalBests.bestDate ? ' · ' + formatDate(personalBests.bestDate) : '') : 'N/A' },
      ...
      <div className="p-3 md:p-4 bg-indigo-50 dark:bg-indigo-950/30 rounded-xl text-center text-[11px] md:text-[12px] font-bold text-indigo-700 dark:text-indigo-300 border border-indigo-100 dark:border-indigo-900/40">
        With Lower Is Better, progress means moving down. Every value below your limit is a win.
      </div>

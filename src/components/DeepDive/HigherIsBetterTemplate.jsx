...
import { getHigherIsBetterAnalytics } from '../../lib/higherIsBetterAnalytics';
...
export default function HigherIsBetterTemplate({ habit, allSummaries, habits, dateRange }) {
  const data = useMemo(() => {
    return getHigherIsBetterAnalytics(habit, allSummaries, dateRange);
  ...
  const { summary, targetValue, unit, formatVal, fmtVal: rawFmtVal, bestDay, improvement, weekdayPattern, maxWeekdayAvg, dailyDetails, personalBests, trend, insights, weakestDay } = data;
  ...
  return (
    <div className="flex flex-col gap-3 md:gap-5 w-full pb-10">
        ...
        <div className="p-3.5 md:p-5 flex items-start gap-4 w-full">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-[11px] md:text-xs text-slate-500 dark:text-slate-400 font-medium leading-snug">
              Target: <span className="text-slate-700 dark:text-slate-200 font-bold">≥ {targetValue || habit.userTarget100 || habit.target100 || habit.targetValue || 0}{unit ? ` ${unit}` : (habit.unit || habit.defaultUnit ? ` ${habit.unit || habit.defaultUnit}` : '')}</span>
              <span className="hidden md:inline"> &nbsp;·&nbsp; </span>
              <br className="md:hidden" />
              Scoring: <span className="text-slate-700 dark:text-slate-200">Higher Is Better</span>
            </p>
          </div>
        </div>
        ...
        {showScoringInfo && (
          <div className="px-3.5 md:px-5 pb-3.5 md:pb-5">
            <div className="bg-indigo-50/50 dark:bg-indigo-950/30 rounded-xl border border-indigo-100/50 dark:border-indigo-900/40 p-3 md:p-4 text-[11px] md:text-[12px] text-slate-600 dark:text-slate-300 leading-relaxed">
              ...
              Reaching <strong className="text-emerald-600">{targetValue > 0 ? (unit ? `${targetValue} ${unit}` : fmtVal(targetValue)) : 'your target'} = 100 pts</strong>.
              Going above target stays at 100. Below target, your score gradually decreases.
              <span className="ml-2 font-semibold text-indigo-600">Move UP ↑ → Score goes UP</span>
            </div>
          </div>
        )}
      ...
      {/* ── 3. SCORE CHART + DONUT ──────────────────────────────────────── */}
      <div id="deep-dive-charts-section-higher" className="w-full flex flex-col gap-3">
        {/* Header */}
      ...
      <div className="p-3 md:p-4 bg-indigo-50 dark:bg-indigo-950/30 rounded-xl text-center text-[11px] md:text-[12px] font-bold text-indigo-700 dark:text-indigo-300 border border-indigo-100 dark:border-indigo-900/40">
        With Higher Is Better, progress means moving up. Reaching your target earns 100 points — never more than 100.
      </div>

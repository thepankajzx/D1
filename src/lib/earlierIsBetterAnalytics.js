...
// earlierIsBetterAnalytics.js
// O(N) single-pass analytics for "Earlier Is Better" habit scoring type (Time)
...
export function getEarlierIsBetterAnalytics(habit, allSummaries, dateRange) {
  ...
  // Time targets are stored in habit.target100 and habit.target0 (or userTarget100)
  let targetValue = habit.target100 !== undefined ? Number(habit.target100) : (habit.userTarget100 !== undefined ? Number(habit.userTarget100) : 480);
  let target0 = habit.target0 !== undefined ? Number(habit.target0) : (habit.userTarget0 !== undefined ? Number(habit.userTarget0) : 600);
  
  if (isNaN(targetValue)) {
    // Attempt to parse "HH:MM"
    const tv = String(habit.target100 || habit.userTarget100 || "08:00");
    const [h, m] = tv.split(':').map(Number);
    targetValue = (h || 8) * 60 + (m || 0);
  }
  if (isNaN(target0)) {
    const t0 = String(habit.target0 || habit.userTarget0 || "10:00");
  ...
  for (let i = 0; i < allSummaries.length; i++) {
    ...
    // Some sleep habits add 1440 if before 12pm, normalize it back for display if needed
    ...
    if (s.id >= start && s.id <= end) {
      if (hasData) {
        ...
        // Earlier is better means LOWER time is better
  ...
  if (daysOnTarget > 0) {
    insights.push(`🎯 You were on time on ${daysOnTarget} of ${diffDays} days (${Math.round(daysOnTarget / diffDays * 100)}%).`);
  }
  if (valueChange !== null && valueChange < 0) {
    const hr = Math.floor(Math.abs(valueChange) / 60);
    const mn = Math.abs(valueChange) % 60;
    const timeStr = hr > 0 ? `${hr}h ${mn}m` : `${mn}m`;
    insights.push(`↗ You averaged ${timeStr} earlier vs last period.`);
  }
